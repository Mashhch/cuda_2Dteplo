...
__global__ void solution(float* x, int Nx, float h_x, float* y, int Ny, float h_y, float* t_prev_layer, float* y_x_layer, int t_i, float c_x, float c_y, float counter_for_cells) {
	...
	__shared__ float shared_block[(BLOCK_SIZE_X + 2)*(BLOCK_SIZE_Y + 2)];
	int i = threadIdx.x + blockIdx.x*blockDim.x;
	int j = threadIdx.y + blockIdx.y*blockDim.y;
	int shared_i = threadIdx.x + 1;
	int shared_j = threadIdx.y + 1;

	shared_block[shared_j*(BLOCK_SIZE_X + 2) + shared_i] = t_prev_layer[j * Nx + i];

	if (threadIdx.x == 0 && j != 0) {
		shared_block[(shared_j)*(BLOCK_SIZE_X + 2) + 0] = t_prev_layer[j * Nx + i - 1];
	}
	if (threadIdx.x == BLOCK_SIZE_X - 1 && j != Ny-1) {
		shared_block[(shared_j)*(BLOCK_SIZE_X + 2) + BLOCK_SIZE_X + 1] = t_prev_layer[j * Nx + i+1];
	}
	if (threadIdx.y == 0 && i != 0) {
		shared_block[0 * (BLOCK_SIZE_X + 2) + shared_i] = t_prev_layer[(j - 1) * Nx + i];
	}
	if (threadIdx.y == BLOCK_SIZE_Y - 1 && i != Nx - 1) {
		shared_block[(BLOCK_SIZE_Y + 1)*(BLOCK_SIZE_X + 2) + shared_i] = t_prev_layer[(j + 1) * Nx + i];
	}
	...
	if (i > 0 && i < Nx - 1 && j > 0 && j < Ny - 1) {
		y_x_layer[j * Nx + i] = (c_x * (shared_block[(shared_j)*(BLOCK_SIZE_X + 2) + shared_i + 1] - 2 * shared_block[(shared_j)*(BLOCK_SIZE_X + 2) + shared_i] + shared_block[(shared_j)*(BLOCK_SIZE_X + 2) + shared_i - 1]) +
			c_y * (shared_block[(shared_j + 1)*(BLOCK_SIZE_X + 2) + shared_i] - 2 * shared_block[(shared_j)*(BLOCK_SIZE_X + 2) + shared_i] + shared_block[(shared_j - 1)*(BLOCK_SIZE_X + 2) + shared_i]) +
			shared_block[(shared_j)*(BLOCK_SIZE_X + 2) + shared_i] + q(x[i], h_x, y[j], h_y, counter_for_cells));
	}
